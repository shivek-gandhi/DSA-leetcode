...
public:
    ...
    pair<ll,ll> solve(int curr,int prevPrev,int prev, bool isLimitedWithActualNumber, bool isLeadingZero){
        if(curr == n){
            return {1,0};
        } // no more score can be generated and we found one last numebr
        ...
        for(int digit=0;digit <= limitDigit;digit++){
            ...
            auto [remainTotalNumbers, remainTotalWaveScore] = solve(curr+1, newPrevPrev, newPrev, isLimitedWithActualNumber && (digit==limitDigit),newIsLeadingZero);
